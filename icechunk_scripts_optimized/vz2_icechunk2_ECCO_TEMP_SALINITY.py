"""
Create Icechunk v2 virtual Zarr store for the ECCO_L4_TEMP_SALINITY_05DEG_DAILY_V4R4 dataset.
...
SHORTNAMES = ["ECCO_L4_TEMP_SALINITY_05DEG_DAILY_V4R4"]

FNAME_STORE_S3 = "ECCO_L4_TEMP_SALINITY_05DEG_DAILY_V4R4.icechunk_v2.s3"
FNAME_STORE_HTTP = "ECCO_L4_TEMP_SALINITY_05DEG_DAILY_V4R4.icechunk_v2.https"
...
def create_dask_cluster(n_workers=8, memory_limit="4GiB"):
...
def create_s3_icechunk_repo_s3access(output_bucket: str, prefix: str, vcc_bucket: str):
    ...
    config.set_virtual_chunk_container(
        icechunk.VirtualChunkContainer(url_prefix=vcc_bucket + "/", store=icechunk.s3_store(region="us-west-2", anonymous=True))
    )
...
def create_s3_icechunk_repo_httpaccess(output_bucket: str, prefix: str, vcc_http_base: str):
    ...
    config = icechunk.RepositoryConfig.default()
    config.set_virtual_chunk_container(
        icechunk.VirtualChunkContainer(vcc_http_base, icechunk.http_store()))
    return icechunk.Repository.create(storage, config)
...
def main(output_bucket: str):
    ...
    vds_composite_s3.attrs['history'] = "Icechunk v2 VDS for ECCO L4 Temp Salinity"
    ...
    repo_http = create_s3_icechunk_repo_httpaccess(output_bucket, http_prefix, "https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-protected/")
...
def cli():
    parser = argparse.ArgumentParser(description="Generate Icechunk v2 stores for ECCO_L4_TEMP_SALINITY_05DEG_DAILY_V4R4")
    parser.add_argument("--output-bucket", type=str, default=os.environ.get("OUTPUT_BUCKET", ""), help="S3 bucket for icechunk stores")
