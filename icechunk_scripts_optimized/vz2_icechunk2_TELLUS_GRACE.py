"""
Create Icechunk v2 virtual Zarr store for the TELLUS_GRAC-GRFO_MASCON_CRI_GRID_RL06.3_V4 dataset.
...
SHORTNAMES = ["TELLUS_GRAC-GRFO_MASCON_CRI_GRID_RL06.3_V4"]

FNAME_STORE_S3 = "TELLUS_GRAC-GRFO_MASCON_CRI_GRID_RL06.3_V4.icechunk_v2.s3"
FNAME_STORE_HTTP = "TELLUS_GRAC-GRFO_MASCON_CRI_GRID_RL06.3_V4.icechunk_v2.https"
...
def main(output_bucket: str):
    ...
    for sn in SHORTNAMES:
        ...
        # 3. Create VDS reference (single granule — use open_virtual_dataset directly)
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message="Numcodecs codecs*", category=UserWarning)
            if len(granule_data_urls_s3) == 1:
                vds_s3 = vz.open_virtual_dataset(
                    url=granule_data_urls_s3[0],
    ...
    vds_composite_s3.attrs['history'] = "Icechunk v2 VDS for TELLUS GRACE/GRACE-FO Mascon"
...
def cli():
    parser = argparse.ArgumentParser(description="Generate Icechunk v2 stores for TELLUS_GRAC-GRFO_MASCON_CRI_GRID_RL06.3_V4")
    parser.add_argument(
        "--output-bucket",
        type=str,
