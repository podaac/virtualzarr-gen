...
python3 generate_vds_clean.py --collection $COLLECTION --loadable-coord-vars $LOADABLE_VARS --start-#!/bin/bash
# Environment Variables must be passed into the docker run command
...
aws s3 sync . s3://$OUTPUT_BUCKET/virtualcollection/$COLLECTION/ --exclude "*" --include "*virtual_https.json"date $startDate --end-date $endDate
